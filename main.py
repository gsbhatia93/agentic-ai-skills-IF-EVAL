...
def get_models():
    return run_no_skills.get_models()


IFEVAL_TASKS = run_no_skills.IFEVAL_TASKS
...
def print_model_grid(model: str, no_skill: dict, with_skill: dict):
    ...
    n  = len(IFEVAL_TASKS)
    s0 = sum(1 for t in IFEVAL_TASKS if no_skill[t["id"]][0])
    s1 = sum(1 for t in IFEVAL_TASKS if with_skill[t["id"]][0])
    _hline(widths, "├", "┼", "┤")
    _row(widths, "", "Score",
         f"{s0}/{n} ({int(100*s0/n)}%)",
         f"{s1}/{n} ({int(100*s1/n)}%)")
...
def print_summary_grid(all_results: dict):
...
def run_all():
    ...
    for model in models:
        ...
        if mode in ("no_skills", "both"):
            print("\n  [without skills]")
            no_skill = run_no_skills.evaluate(model)
        else:
            no_skill = {t["id"]: (False, "skipped") for t in IFEVAL_TASKS}

        if mode in ("skills", "both"):
            print("\n  [with skills]")
            with_skill = run_with_skills.evaluate(model)
        else:
            with_skill = {t["id"]: (False, "skipped") for t in IFEVAL_TASKS}
    ...
    for model, (no_skill, with_skill) in all_results.items():
        print_model_grid(model, no_skill, with_skill)

    print_summary_grid(all_results)
